			...
			struct i2c_msg msgs[], int num)
{
		...
		if (((transaction.status_error & IIC_E_NOTACK)
		    ...
		    || (transaction.status_error & IIC_E_BUSY)) {
				...
				if (transaction.status_error & IIC_E_NOTACK) {
					...
					/*
					 * Some HDMI Sinks remain stucked at
					 * this stage, try to generate a STOP
					 * manually...
					 */
					iic_pio_stop(adap);
					result = -EBUSY;
	...
	/* Disable transmission and reception to avoid glitches */
	ssc_store32(adap, SSC_CTL, ssc_load32(adap, SSC_CTL) & ~SSC_CTL_EN);
