...
/* Sequence for diabling EEE on PHY ports for RTL8367RB switch */
static int rtl8367rb_disable_eee_phy_port(struct rtl8366_smi *smi, int port)
{
	int err; /* for REG_WR API */
	u32 reg;

	reg = (0x20 * port);

	REG_WR(smi, 0x18 + reg, 0x00);
	REG_WR(smi, RTL8367_IA_WRITE_DATA_REG, 0x0000);
	REG_WR(smi, RTL8367_IA_ADDRESS_REG, 0x201f + reg);
	REG_WR(smi, RTL8367_IA_CTRL_REG, 0x0003);
	REG_WR(smi, RTL8367_IA_WRITE_DATA_REG, 0x0007);
	REG_WR(smi, RTL8367_IA_ADDRESS_REG, 0x200d + reg);
	REG_WR(smi, RTL8367_IA_CTRL_REG, 0x0003);
	REG_WR(smi, RTL8367_IA_WRITE_DATA_REG, 0x003C);
	REG_WR(smi, RTL8367_IA_ADDRESS_REG, 0x200e + reg);
	REG_WR(smi, RTL8367_IA_CTRL_REG, 0x0003);
	REG_WR(smi, RTL8367_IA_WRITE_DATA_REG, 0x4007);
	REG_WR(smi, RTL8367_IA_ADDRESS_REG, 0x200d + reg);
	REG_WR(smi, RTL8367_IA_CTRL_REG, 0x0003);
	REG_WR(smi, RTL8367_IA_WRITE_DATA_REG, 0x0000);
	REG_WR(smi, RTL8367_IA_ADDRESS_REG, 0x200e + reg);
	...
	REG_WR(smi, RTL8367_IA_ADDRESS_REG, 0x201f + reg);
	REG_WR(smi, RTL8367_IA_CTRL_REG, 0x0003);
	REG_WR(smi, RTL8367_IA_WRITE_DATA_REG, 0x1340);
	REG_WR(smi, RTL8367_IA_ADDRESS_REG, 0x2000 + reg);
	REG_WR(smi, RTL8367_IA_CTRL_REG, 0x0003);
...
static int rtl8367_setup(struct rtl8366_smi *smi)
{
	...
	if (of_device_is_compatible(np, "realtek,rtl8367rb")) {
		int port;
		/* By default disable EEE support for all ports, this is due to
		 * some HW instabilities and packet corruptions found while
		 * connecting STi platforms to some EEE partner capable.
		 */
		for (port = 0; port < RTL8367RB_NUM_PORTS; port++)
			err = rtl8367rb_disable_eee_phy_port(smi, port);
			if (err) {
				dev_err(smi->parent, "Error disabling EEE.\n");
				return err;
			}
	}
	return 0;
