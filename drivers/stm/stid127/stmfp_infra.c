...
int stid127_fp_claim_resources(void *plat)
{
	...
	sc_rst = sysconf_claim(SYSCONF(706), 28, 28, "fp_reset");
	if (!sc_rst) {
		pr_err("%s:unable to claim sysconf rst_level3_n\n", __func__);
		return -EBUSY;
	}
	sysconf_write(sc_rst, 0);
	sysconf_write(sc_rst, 1);
	sysconf_release(sc_rst);
