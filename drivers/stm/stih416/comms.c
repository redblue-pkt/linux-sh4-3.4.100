	...
	[0] = {
		...
		.capt_pad_config = {
			[0] = &(struct stm_pad_config) {
				.gpios_num = 1,
				.gpios = (struct stm_pad_gpio []) {
					STIH416_PAD_PIO_IN(9, 6, 2),
				},
			},
			[1] = &(struct stm_pad_config) {
				.gpios_num = 1,
				.gpios = (struct stm_pad_gpio []) {
					STIH416_PAD_PIO_IN(13, 1, 2),
				},
			},
		},
	},
	...
	[1] = {
		...
		.capt_pad_config = {
			[0] = &(struct stm_pad_config) {
				.gpios_num = 1,
				.gpios = (struct stm_pad_gpio []) {
					STIH416_PAD_PIO_IN(3, 2, 1),
				},
			},
			[1] = &(struct stm_pad_config) {
				.gpios_num = 1,
				.gpios = (struct stm_pad_gpio []) {
					STIH416_PAD_PIO_IN(4, 3, 1),
				},
			},
		},
	},
		...
		struct stih416_pwm_config *config)
{
	...
	for (channel = 0; channel < ARRAY_SIZE(config->out_channel_config);
	     channel++)
		stih416_pwm_platform_data[pwm].out_channel_config[channel] =
			config->out_channel_config[channel];

	for (channel = 0; channel < ARRAY_SIZE(config->capt_channel_config);
	     channel++)
		stih416_pwm_platform_data[pwm].capt_channel_config[channel] =
