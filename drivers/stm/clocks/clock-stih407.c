...
20/dec/13 fabrice.charpentier@st.com
...
06/dec/13 fabrice.charpentier@st.com
	  enable()/disable() & set_rate() revisited for PCM_x/SPDIFF clocks.
14/oct/13 fabrice.charpentier@st.com
	  Added 'fclkgen_set_parent()' to filter clocks passed to flexgen lib.
20/sep/13 fabrice.charpentier@st.com
	  PLL3200 clocks now bypassed during reprogramation.
13/sep/13 fabrice.charpentier@st.com
	  flexiclockgen lib update for semi-sync clocks.
11/sep/13 fabrice.charpentier@st.com
	  flexgen_fs660c32_xable() revisited.
07/sep/13 fabrice.charpentier@st.com
	  Added support for semi synch clocks
06/sep/13 fabrice.charpentier@st.com
	  Added support for some more clock obs thu je_inputs.
03/sep/13 fabrice.charpentier@st.com
	  Clock obs bug fixes.
29/aug/13 fabrice.charpentier@st.com
	  FS660 setup bug fix.
06/aug/13 fabrice.charpentier@st.com
	  Some clock name alignment with prog manual. Flexiclockgen lib
	  update.
02/aug/13 fabrice.charpentier@st.com
	  Clockgen D2 revisited for HDMI TMDS in/out & CkPxPLL.
01/aug/13 fabrice.charpentier@st.com
	  Added CLK_USB2_PHY ext_refclkout support.
24/jul/13 fabrice.charpentier@st.com
	  Fully revisited to move generic flexgen code to "clk-flexgen.c/h"
12/jul/13 fabrice.charpentier@st.com
	  Adding "struct flexgen" for better genericity.
11/jul/13 fabrice.charpentier@st.com
	  flexgen_pll3200c32_xable() + flexgen_fs660c32_xable updates.
10/jul/13 fabrice.charpentier@st.com
	  Plenty updates + support of coemul injected clocks
xx/jun/13 ravinder-dlh.singh@st.com
	  Updates for configuring the GFG crossbar and dividers.
13/jun/13 fabrice.charpentier@st.com
	  Revisited & prelim updates for Linux compilation.
*/
...
#include "clock-stih407.h"
#include "clock-regs-stih407.h"
#include "clock-oslayer.h"
#include "clk-common.h"
#include "clk-flexgen.h"

struct sysconf_field *stih407_sysconf_claim(int num, int lsb, int msb)
{
	return sysconf_claim(SYSCONFG_GROUP(num), SYSCONF_OFFSET(num),
		lsb, msb, "Clk lla");
}
#define SYSCONF_CLAIMU stih407_sysconf_claim
...
/*	Clockgen "A0"
	...
	Clockgen "C0"
		...
		FG_REFIN, CLK_C0_REF
		FG_DIV, CLK_ICN_GPU, CLK_COMPO_DVP
	Clockgen "D0"
		GFGs
			FG_FS660C32
...
/* No observation function defined under Linux */
...
_CLK(CLK_TX_ICN_DISP_0,	&fclkgenc0,   300000000,    CLK_ALWAYS_ENABLED),
...
/* ========================================================================
   Name:        stih407_plat_clk_init()
   Description: SOC specific LLA initialization
   Returns:     'clk_err_t' error code.
   ======================================================================== */

int __init stih407_plat_clk_init(void)
{
	int err;

	pr_info("H407 clock LLA %s\n", LLA_VERSION);
	...
	clk_prepare_enable(&clk_clocks[CLK_MMC_0]);
	clk_set_rate(&clk_clocks[CLK_MMC_0], 200000000);
