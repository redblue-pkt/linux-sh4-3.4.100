...
25/apr/13 Fabrice Charpentier
	  Some resync with Linux + moved inits struct at EOF to
	    remove prototypes + clkgenf_fsyn_recalc() bug fix.
	  clkgenf_Xable() enhanced.
04/apr/13 Fabrice Charpentier
	  clkgenax_enable() update to allow enable when no parent set.
14/dec/12 Fabrice Charpentier
	  Upgraded prog sequence for clockgen E & F (nsb before EN prog).
30/oct/12 Fabrice Charpentier
	  Migrated FS660 to new 'clk-common' API.
20/sep/12 Francesco Virlinzi
	  New clock common API applied on PLL3200 & PLL1200.
...
static int clkgenax_recalc(clk_t *clk_p)
{
	...
	case CLKM_A2_PLL0:
		...
		pll.ndiv = CLK_READ(base_addr + CKGA_PLL0_REG0_CFG) & 0xff;
		pll.idf = CLK_READ(base_addr + CKGA_PLL0_REG1_CFG) & 0x7;
		err = stm_clk_pll_get_rate(clk_p->parent->rate, &pll, &clk_p->rate);
		#endif
		break;
	case CLKM_A0_PLL1:
	case CLKM_A1_PLL1:
	case CLKM_A2_PLL1:
		#if !defined(CLKLLA_NO_PLL)
		pll.ndiv = CLK_READ(base_addr + CKGA_PLL1_REG0_CFG) & 0xff;
		pll.idf = CLK_READ(base_addr + CKGA_PLL1_REG1_CFG) & 0x7;
		err = stm_clk_pll_get_rate(clk_p->parent->rate, &pll, &clk_p->rate);
		#endif
		break;
...
static int clkgenax_enable(clk_t *clk_p)
{
	...
	parent_clk = (clk_p->id >= CLKM_A2_REF) ? &clk_clocks[CLKM_A2_REF]
		      : (clk_p->id >= CLKM_A1_REF) ?
		      &clk_clocks[CLKM_A1_REF] : &clk_clocks[CLKM_A0_REF];
...
static int clkgena2_set_rate(clk_t *clk_p, unsigned long freq)
{
	...
	case CLKM_A2_PLL0:
		...
		break;
	case CLKM_A2_PLL1:
		err = stm_clk_pll_get_params(clk_p->parent->rate,
			freq, &pll);
		if (err != 0)
			break;
		#if !defined(CLKLLA_NO_PLL)
...
static int clkgene_fsyn_recalc(clk_t *clk_p)
{
	...
	fs_vco.ndiv = SYSCONF_READ(0, 562, 24, 26);
	if (clk_p->id == CLKM_E_FS_VCO)
		return stm_clk_fs_get_rate(clk_p->parent->rate, &fs_vco,
					      &clk_p->rate);
	...
	fs.mdiv = SYSCONF_READ(0, 562, 4, 23);
	fs.mdiv >>= (5 * chan);
	fs.mdiv &= 0x1f;

	switch (chan) {
	case 0:	fs.pe = SYSCONF_READ(0, 563, 4, 18); break;
	case 1:	fs.pe = SYSCONF_READ(0, 564, 0, 14); break;
	case 2:	fs.pe = SYSCONF_READ(0, 564, 15, 29); break;
	default:
		fs.pe = SYSCONF_READ(0, 565, 0, 14);
	}

	fs.sdiv = SYSCONF_READ(0, 565, 16, 31);
	fs.sdiv >>= (4 * chan);
	fs.sdiv &= 0xf;
...
static int clkgene_fsyn_set_rate(clk_t *clk_p, unsigned long freq)
{
	...
	if (clk_p->id == CLKM_E_FS_VCO) {
		if (stm_clk_fs_get_params(clk_p->parent->rate, freq, &fs_vco))
			return CLK_ERR_BAD_PARAMETER;
		SYSCONF_WRITE(0, 562, 24, 26, fs_vco.ndiv);
	...
	switch (chan) {
	case 0:
		SYSCONF_WRITE(0, 563,  4, 18, fs.pe);
		break;
	case 1:
		SYSCONF_WRITE(0, 564, 0, 14, fs.pe);
		break;
	case 2:
		SYSCONF_WRITE(0, 564, 15, 29, fs.pe);
		break;
	default:
		SYSCONF_WRITE(0, 565, 0, 14, fs.pe);
	}
...
static int clkgenf_fsyn_recalc(clk_t *clk_p)
{
	...
	struct stm_fs fs = {
		.type = stm_fs660c32,
		.nsdiv = 1 /* All channels hardwired to '1' */
	};
	...
	/* At least analog part (PLL660) is running */
	fs_vco.ndiv = SYSCONF_READ(0, 558, 24, 26);
	...
	/* FSYN is up and running.
	   Now computing frequency */
	fs.mdiv = SYSCONF_READ(0, 558, 4, 23);
	fs.mdiv = (fs.mdiv >> (5 * chan)) & 0x1f;

	switch (chan) {
	case 0:	fs.pe = SYSCONF_READ(0, 559, 4, 18); break;
	case 1:	fs.pe = SYSCONF_READ(0, 560, 0, 14); break;
	case 2:	fs.pe = SYSCONF_READ(0, 560, 15, 29); break;
	default:
		fs.pe = SYSCONF_READ(0, 561, 0, 14);
	}

	fs.sdiv = SYSCONF_READ(0, 561, 16, 31);
	fs.sdiv = (fs.sdiv >> (4 * chan)) & 0xf;
	return stm_clk_fs_get_rate(clk_p->parent->rate, &fs, &clk_p->rate);
}
...
static int clkgenf_fsyn_set_rate(clk_t *clk_p, unsigned long freq)
{
	...
	if (clk_p->id == CLKM_F_FS_VCO) {
		if (stm_clk_fs_get_params(clk_p->parent->rate, freq, &fs_vco))
			return CLK_ERR_BAD_PARAMETER;
		SYSCONF_WRITE(0, 558, 24, 26, fs_vco.ndiv);
	...
	switch (chan) {
	case 0:
		SYSCONF_WRITE(0, 559, 4, 18, fs.pe);
		break;
	case 1:
		SYSCONF_WRITE(0, 560, 0, 14, fs.pe);
		break;
	case 2:
		SYSCONF_WRITE(0, 560, 15, 29, fs.pe);
		break;
	default:
		SYSCONF_WRITE(0, 561, 0, 14, fs.pe);
		break;
	}
...
static int clkgenf_enable(clk_t *clk_p)
{
	...
	case CLKM_F_REF:
		return 0;
	/* FS outputs */
	case CLKM_F_FS_VCO ... CLKM_FVDP_PROC_FS:
		return clkgenf_fsyn_xable(clk_p, 1);
	/* Muxes outputs */
	case CLKM_FVDP_PROC:
		return clkgenf_enable(clk_p->parent);
	/* Part of clockgen but do nothing */
	case CLKM_PIX_MAIN_SAS ... CLKM_PIX_AUX_PIPE:
		break;
	}
...
static int clkgenddr_recalc(clk_t *clk_p)
{
	...
	case CLKM_DDR_IC_LMI0 ...CLKM_DDR_IC_LMI1:
		...
		pll.idf = SYSCONF_READ(0, 601, 25, 27);
		pll.ndiv = SYSCONF_READ(0, 603, 0, 7);
...
static int clkgena9_recalc(clk_t *clk_p)
{
	...
	if (clk_p->id == CLKM_A9_REF || clk_p->id == CLKM_A9)
		...
		pll.idf = SYSCONF_READ(0, 654, 22, 24);
		pll.ndiv = SYSCONF_READ(0, 654, 9, 16);
...
static int clkgena9_set_rate(clk_t *clk_p, unsigned long freq)
{
	...
	SYSCONF_WRITE(0, 654, 22, 24, pll.idf);	/* IDF: syscfg654, bits 22 to 24 */
	SYSCONF_WRITE(0, 654, 9, 16, pll.ndiv);	/* NDIV: syscfg654, bits 9 to 16 */
...
static int clkgengpu_recalc(clk_t *clk_p)
{
	...
	if (clk_p->id == CLKM_GPU_REF || clk_p->id == CLKM_GPU)
		...
		/* PLL is ON */
		pll.idf = CLK_READ(mali_base + 0) & 0x3;
		pll.ldf = (CLK_READ(mali_base + 0) >> 3) & 0x7f;
		pll.odf = (CLK_READ(mali_base + 0) >> 10) & 0x3f;
