			...
			struct device_node *child)
{
	...
	/* Check if we need reparenting, then enable and set the p-rate */
	...
	if (!ret) {
		...
		/* Change prate only if re-parenting was ok */
		if (prate)
			clk_set_rate(pclk, prate);
	}

	/* Clock must be kept disabled after re-parenting, if the case */
	if (disable) {
